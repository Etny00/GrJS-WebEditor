grapesjs.plugins.add('gjs-sections', function(editor, opts) {
  ...
  domt.addType('cs-hero-section', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-hero-section') || (el.hasAttribute && el.hasAttribute('data-hero-bg')) ? { type: 'cs-hero-section' } : undefined; },
    model: {
      defaults: {
        tagName: 'section',
        traits: [
          { type: 'select', name: 'data-hero-bg', label: 'Background', options: [{ id: 'gradient', name: 'Purple Gradient' }, { id: 'dark', name: 'Dark' }, { id: 'light', name: 'Light' }, { id: 'white', name: 'White' }] },
          { type: 'checkbox', name: 'data-hero-fullheight', label: 'Full Height (100vh)', valueTrue: '1', valueFalse: '0' },
          { type: 'select', name: 'data-hero-align', label: 'Text Align', options: [{ id: 'center', name: 'Center' }, { id: 'left', name: 'Left' }, { id: 'right', name: 'Right' }] }
        ]
      }
    }
  });
  ...
  domt.addType('cs-before-after', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-before-after') || (el.hasAttribute && el.hasAttribute('data-ba-accent')) ? { type: 'cs-before-after' } : undefined; },
    model: { defaults: { tagName: 'section', traits: [
      { type: 'text', name: 'data-ba-label-before', label: 'Label Links (Before)' },
      { type: 'text', name: 'data-ba-label-after', label: 'Label Rechts (After)' },
      { type: 'color', name: 'data-ba-accent', label: 'Divider-Farbe' },
      { type: 'color', name: 'data-ba-bg', label: 'Hintergrund' }
    ] } }
  });

  domt.addType('cs-portfolio', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-portfolio') || (el.hasAttribute && el.hasAttribute('data-pf-cols')) ? { type: 'cs-portfolio' } : undefined; },
    model: { defaults: { tagName: 'section', traits: [
      { type: 'select', name: 'data-pf-cols', label: 'Columns', options: [{ id:'2', name:'2 Columns' }, { id:'3', name:'3 Columns' }, { id:'4', name:'4 Columns' }] },
      { type: 'color', name: 'data-pf-accent', label: 'Overlay Color' },
      { type: 'color', name: 'data-pf-bg', label: 'Background' },
      { type: 'select', name: 'data-pf-radius', label: 'Card Radius', options: [{ id:'0px', name:'None' }, { id:'6px', name:'Small' }, { id:'10px', name:'Medium' }, { id:'20px', name:'Large' }] }
    ] } }
  });

  domt.addType('cs-faq', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-faq') || (el.hasAttribute && el.hasAttribute('data-faq-accent')) ? { type: 'cs-faq' } : undefined; },
  ...
  domt.addType('cs-newsletter', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-newsletter') || (el.hasAttribute && el.hasAttribute('data-nl-btncolor')) ? { type: 'cs-newsletter' } : undefined; },
    model: { defaults: { tagName: 'div', traits: [
      { type: 'text', name: 'data-nl-btntext', label: 'Button Text' },
      { type: 'text', name: 'data-nl-placeholder', label: 'Input Placeholder' },
      { type: 'color', name: 'data-nl-btncolor', label: 'Button Color' }
    ] } }
  });

  domt.addType('cs-cookie', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-cookie') || (el.hasAttribute && el.hasAttribute('data-cookie-text')) ? { type: 'cs-cookie' } : undefined; },
    model: { defaults: { tagName: 'div', traits: [
  ...
  domt.addType('cs-navbar', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-navbar') || (el.hasAttribute && el.hasAttribute('data-nav-bg')) ? { type: 'cs-navbar' } : undefined; },
    model: { defaults: { tagName: 'div', traits: [
      { type: 'text', name: 'data-nav-logo', label: 'Logo Text' },
      { type: 'color', name: 'data-nav-bg', label: 'Nav Background' },
      { type: 'color', name: 'data-nav-linkcolor', label: 'Link Color' },
      { type: 'select', name: 'data-nav-pos', label: 'Position', options: [{ id:'sticky', name:'Sticky' }, { id:'fixed', name:'Fixed' }, { id:'relative', name:'Static' }] }
    ] } }
  });

  domt.addType('cs-pricing', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-pricing') || (el.hasAttribute && el.hasAttribute('data-pricing-currency')) ? { type: 'cs-pricing' } : undefined; },
    model: { defaults: { tagName: 'div', traits: [
      { type: 'select', name: 'data-pricing-currency', label: 'Currency', options: [{ id:'$', name:'$ Dollar' }, { id:'€', name:'€ Euro' }, { id:'£', name:'£ Pound' }, { id:'¥', name:'¥ Yen' }] },
      { type: 'color', name: 'data-pricing-color', label: 'Highlight Color' }
    ] } }
  });

  domt.addType('cs-map', {
    isComponent: function(el) { return (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-map') || (el.hasAttribute && el.hasAttribute('data-map-url')) ? { type: 'cs-map' } : undefined; },
    model: { defaults: { tagName: 'div', traits: [
      { type: 'text', name: 'data-map-url', label: 'IFrame URL' },
      { type: 'text', name: 'data-map-address', label: 'Address Text' },
      { type: 'text', name: 'data-map-phone', label: 'Phone Text' },
      { type: 'color', name: 'data-map-sidecolor', label: 'Sidebar Color' }
    ] } }
  });
  ...
  bm.add('portfolio-grid', {
    ...
    content: {
      ...
      components: [
        {
          tagName: 'div',
          attributes: { style: 'max-width:1100px;margin:0 auto;' },
          components: [
            ...
            {
              tagName: 'div',
              attributes: { style: 'display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-bottom:36px;' },
              components: [
            ...
            {
              tagName: 'div',
              attributes: { class: 'cs-pf-grid', style: 'display:grid;grid-template-columns:repeat(3,1fr);gap:20px;' },
              components: [
  ...
  bm.add('map-embed', {
    label: 'Map Embed', category: 'Sections', attributes: { class: 'fa fa-map-marker' },
    content: {
      type: 'cs-map',
      attributes: { 'data-gjs-type': 'cs-map', 'data-map-url': 'https://maps.google.com/maps?q=Berlin&t=&z=13&ie=UTF8&iwloc=&output=embed', 'data-map-address': '123 Business Ave, Berlin, Germany', 'data-map-phone': '+49 30 123456', 'data-map-sidecolor': '#4d114f' },
      components: '<section style="display:flex;flex-wrap:wrap;background:#fff;font-family:Helvetica,sans-serif;min-height:400px;"><div style="flex:1;min-width:300px;position:relative;"><iframe src="https://maps.google.com/maps?q=Berlin&t=&z=13&ie=UTF8&iwloc=&output=embed" frameborder="0" style="position:absolute;inset:0;width:100%;height:100%;border:none;"></iframe></div><div style="width:300px;background:#4d114f;color:#fff;padding:50px 40px;display:flex;flex-direction:column;justify-content:center;flex-shrink:0;"><h3 style="margin:0 0 20px;font-size:1.5em;font-weight:700;">Visit Us</h3><p style="margin:0 0 30px;line-height:1.6;opacity:.9;"><strong>Location:</strong><br>123 Business Ave, Berlin, Germany</p><p style="margin:0 0 30px;line-height:1.6;opacity:.9;"><strong>Phone:</strong><br>+49 30 123456</p><div style="display:flex;gap:15px;"><a href="#" style="color:#fff;font-size:1.2em;"><i class="fa fa-facebook"></i></a><a href="#" style="color:#fff;font-size:1.2em;"><i class="fa fa-twitter"></i></a><a href="#" style="color:#fff;font-size:1.2em;"><i class="fa fa-instagram"></i></a></div></div></section>'
    }
  });
  ...
  bm.add('quote-66', {
    label: 'Quote 66', category: 'Sections', attributes: { class: 'fa fa-quote-right' },
    content: '<section style="padding:80px 20px;background:#fff;font-family:Helvetica,sans-serif;text-align:center;"><div style="max-width:800px;margin:0 auto;"><div style="font-size:6em;line-height:1;color:#f0e0f0;margin-bottom:-0.2em;font-family:Georgia,serif;">&ldquo;</div><blockquote style="margin:0;padding:0;border:none;"><p style="font-size:1.5em;font-style:italic;color:#333;line-height:1.5;margin-bottom:30px;">Design is not just what it looks like and feels like. Design is how it works.</p><footer style="font-size:1.1em;font-weight:700;color:#4d114f;">&mdash; Steve Jobs</footer></blockquote></div></section>'
  });
  ...
  editor.on('component:add', function(model) {
    ...
    // Navbar
    if ('data-nav-bg' in attrs) {
      model.on('change:attributes', function(m) {
        var changed = (m.changed && m.changed.attributes) || {};
        var a = model.getAttributes();
        var header = child('header');
        if ('data-nav-bg' in changed) header.addStyle({ 'background': a['data-nav-bg'] || '#ffffff' });
        if ('data-nav-linkcolor' in changed) {
          var color = a['data-nav-linkcolor'] || '#555555';
          if (header.components) {
            header.components().each(function(c) { if (c.get('tagName') === 'a') c.addStyle({ 'color': color }); });
          }
        }
        if ('data-nav-pos' in changed) header.addStyle({ 'position': a['data-nav-pos'] || 'sticky' });
        if ('data-nav-logo' in changed) {
          var logoLink = null;
          if (header.components) {
            header.components().find(function(c) {
              if (c.get('tagName') === 'div' && c.components) {
                c.components().each(function(cc) { if (cc.get('tagName') === 'a' && !logoLink) logoLink = cc; });
              }
            });
          }
    ...
    // FAQ
    if ('data-faq-accent' in attrs) {
      model.on('change:attributes', function(m) {
        var changed = (m.changed && m.changed.attributes) || {};
        var a = model.getAttributes();
        var section = child('section');
        if ('data-faq-bg' in changed) section.addStyle({ 'background': a['data-faq-bg'] || '#f9f9f9' });
        if ('data-faq-accent' in changed) {
          var el = section.getEl && section.getEl();
          if (el) {
            var spans = el.querySelectorAll('span[style*="color"]');
            for (var i = 0; i < spans.length; i++) spans[i].style.color = a['data-faq-accent'];
          }
        }
      });
    }
      ...
      model.on('change:attributes', function(m) {
        ...
        if ('data-map-address' in changed) {
          ...
          for (var i = 0; i < ps.length; i++) {
            if (ps[i].innerHTML.indexOf('Location') !== -1) {
              var br = ps[i].querySelector('br');
              if (br && br.nextSibling) br.nextSibling.textContent = a['data-map-address'];
            }
          }
        }
        if ('data-map-phone' in changed) {
          var ps2 = el.querySelectorAll('p');
          for (var j = 0; j < ps2.length; j++) {
            if (ps2[j].innerHTML.indexOf('Phone') !== -1) {
              var br2 = ps2[j].querySelector('br');
              if (br2 && br2.nextSibling) br2.nextSibling.textContent = a['data-map-phone'];
            }
          }
        }
        if ('data-map-sidecolor' in changed) {
          var side = el.querySelector('div[style*="width:300px"]');
          if (side) side.style.background = a['data-map-sidecolor'] || '#4d114f';
        }
      });
    }
    ...
    if ('data-pf-accent' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        var a = model.getAttributes();
        var section = child('section');
        if ('data-pf-bg' in changed) section.addStyle({ 'background': a['data-pf-bg'] || '#ffffff' });
        var el = section.getEl && section.getEl();
        if (!el) return;
        ...
        if ('data-pf-accent' in changed) {
          ...
          for (var i = 0; i < ovs.length; i++) ovs[i].style.background = color + 'D1'; // Adding some alpha
          var btns = el.querySelectorAll('button[onclick*=filterPF]');
          for (var j = 0; j < btns.length; j++) {
            if (btns[j].style.background !== 'rgb(255, 255, 255)' && btns[j].style.background !== '#fff') btns[j].style.background = color;
          }
        }
        if ('data-pf-radius' in changed) {
          var items = el.querySelectorAll('.cs-pf-item');
          for (var k = 0; k < items.length; k++) items[k].style.borderRadius = a['data-pf-radius'] || '10px';
        }
      });
    }
