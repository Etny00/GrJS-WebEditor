grapesjs.plugins.add('gjs-sections', (editor, opts = {}) => {
  ...
  domt.addType('cs-hero-section', {
    isComponent: el => el.hasAttribute && el.hasAttribute('data-hero-bg') ? { type: 'cs-hero-section' } : undefined,
    model: {
      defaults: {
        tagName: 'section',
        traits: [
          {
            ...
            label: 'Background',
            options: [
              { id: 'gradient', name: 'Purple Gradient' },
              { id: 'dark',     name: 'Dark' },
              { id: 'light',    name: 'Light' },
              { id: 'white',    name: 'White' },
            ],
  ...
  domt.addType('cs-before-after', {
    isComponent: el => (el.hasAttribute && el.hasAttribute('data-ba-accent')) || (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-before-after') ? { type: 'cs-before-after' } : undefined,
  ...
  domt.addType('cs-portfolio', {
    isComponent: el => (el.hasAttribute && el.hasAttribute('data-pf-cols')) || (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-portfolio') ? { type: 'cs-portfolio' } : undefined,
    model: { defaults: { tagName: 'section', traits: [
      { type: 'select', name: 'data-pf-cols',   label: 'Columns', changeProp: 0,
      ...
      { type: 'select', name: 'data-pf-radius', label: 'Card Radius', changeProp: 0,
        options: [{ id:'0px', name:'None' }, { id:'6px', name:'Small' }, { id:'10px', name:'Medium' }, { id:'20px', name:'Large' }] },
    ] } },
  });

  domt.addType('cs-faq', {
    isComponent: el => (el.hasAttribute && el.hasAttribute('data-faq-accent')) || (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-faq') ? { type: 'cs-faq' } : undefined,
  ...
  domt.addType('cs-newsletter', {
    isComponent: el => (el.hasAttribute && el.hasAttribute('data-nl-btncolor')) || (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-newsletter') ? { type: 'cs-newsletter' } : undefined,
    model: { defaults: { tagName: 'div', traits: [
  ...
  domt.addType('cs-cookie', {
    isComponent: el => (el.hasAttribute && el.hasAttribute('data-cookie-text')) || (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-cookie') ? { type: 'cs-cookie' } : undefined,
    model: { defaults: { tagName: 'div', traits: [
  ...
  domt.addType('cs-navbar', {
    isComponent: el => (el.hasAttribute && el.hasAttribute('data-nav-bg')) || (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-navbar') ? { type: 'cs-navbar' } : undefined,
    model: { defaults: { tagName: 'div', traits: [
      ...
      { type: 'select', name: 'data-nav-pos',       label: 'Position', changeProp: 0,
        options: [{ id:'sticky', name:'Sticky' }, { id:'fixed', name:'Fixed' }, { id:'relative', name:'Static' }] },
    ] } },
  });

  domt.addType('cs-pricing', {
    isComponent: el => (el.hasAttribute && el.hasAttribute('data-pricing-currency')) || (el.getAttribute && el.getAttribute('data-gjs-type') === 'cs-pricing') ? { type: 'cs-pricing' } : undefined,
    model: { defaults: { tagName: 'div', traits: [
      { type: 'select', name: 'data-pricing-currency', label: 'Currency', changeProp: 0,
        options: [{ id:'$', name:'$ Dollar' }, { id:'€', name:'€ Euro' }, { id:'£', name:'£ Pound' }, { id:'¥', name:'¥ Yen' }] },
  ...
  bm.add('hero-section', {
    ...
    content: {
      type: 'cs-hero-section',
      tagName: 'section',
      attributes: {
        'data-gjs-type': 'cs-hero-section',
        'data-hero-fullheight': '0',
        'data-hero-bg': 'gradient',
        style: 'background:linear-gradient(135deg,#4d114f,#a0449e);padding:100px 20px;text-align:center;color:#fff;font-family:Helvetica,sans-serif;min-height:auto;box-sizing:border-box;'
      },
      components: [
        {
          tagName: 'div',
          attributes: { style: 'max-width:700px;margin:0 auto;' },
          components: [
            { tagName: 'h1', attributes: { style: 'font-size:2.8em;font-weight:700;margin-bottom:20px;' }, components: [{ type: 'text', content: 'Your Headline Here' }] },
            { tagName: 'p',  attributes: { style: 'font-size:1.2em;opacity:.85;margin-bottom:35px;line-height:1.6;' }, components: [{ type: 'text', content: 'A short subheading that explains your value proposition in one sentence.' }] },
            { tagName: 'a',  attributes: { href: '#', style: 'display:inline-block;padding:14px 36px;background:#fff;color:#4d114f;border-radius:4px;font-weight:700;text-decoration:none;' }, components: [{ type: 'text', content: 'Get Started' }] }
          ]
        }
      ],
    },
  ...
  bm.add('faq-section', {
    ...
    content: {
      ...
      traits: [
        { type: 'color', name: 'data-faq-accent', label: 'Accent Color' },
        { type: 'color', name: 'data-faq-bg',     label: 'Background Color' },
      ],
    },
  });
  ...
  bm.add('pricing-table', {
    ...
    content: {
      ...
      attributes: { 'data-gjs-type': 'cs-pricing',
        'data-pricing-currency': '$', 'data-pricing-color': '#4d114f' },
      ...
      traits: [
        { type: 'select', name: 'data-pricing-currency', label: 'Currency',
          options: [{ id: '$', name: '$ Dollar' }, { id: '€', name: '€ Euro' }, { id: '£', name: '£ Pound' }, { id: '¥', name: '¥ Yen' }] },
        { type: 'color', name: 'data-pricing-color', label: 'Highlight Color' },
      ],
    },
  });
  ...
  bm.add('newsletter-section', {
    ...
    content: {
      ...
      traits: [
        { type: 'text',  name: 'data-nl-btntext',     label: 'Button Text',        placeholder: 'Subscribe' },
        { type: 'text',  name: 'data-nl-placeholder', label: 'Input Placeholder',  placeholder: 'Enter your email...' },
        { type: 'color', name: 'data-nl-btncolor',    label: 'Button Color' },
      ],
    },
  });
  ...
  bm.add('before-after-section', {
    ...
    content: {
      ...
      tagName: 'section',
      attributes: {
        'data-gjs-type': 'cs-before-after',
        'data-ba-label-before': 'Before',
        'data-ba-label-after':  'After',
        'data-ba-accent':       '#4d114f',
        'data-ba-bg':           '#f9f9f9',
        style: 'padding:70px 20px;background:#f9f9f9;font-family:Helvetica,sans-serif;text-align:center;',
      },
      components: [
        { tagName: 'div', attributes: { style: 'max-width:800px;margin:0 auto;' },
          components: [
            { tagName: 'h2', attributes: { style: 'font-size:2em;font-weight:700;color:#333;margin-bottom:12px;' }, components: [{ type: 'text', content: 'Before & After' }] },
            { tagName: 'p',  attributes: { style: 'color:#888;margin-bottom:40px;' }, components: [{ type: 'text', content: 'See the difference our solution makes.' }] },
            { tagName: 'div', attributes: { style: 'position:relative;width:100%;height:400px;overflow:hidden;border-radius:12px;box-shadow:0 4px 24px rgba(0,0,0,.12);user-select:none;', id: 'cs-ba-wrap' },
              components: [
                { tagName: 'div', attributes: { style: 'position:absolute;inset:0;background:url(https://picsum.photos/seed/before/800/400) center/cover;' } },
                { tagName: 'div', attributes: { style: 'position:absolute;inset:0;background:url(https://picsum.photos/seed/after/800/400) center/cover;clip-path:inset(0 50% 0 0);', id: 'cs-ba-after' } },
                { tagName: 'div', attributes: { style: 'position:absolute;top:0;bottom:0;left:50%;width:3px;background:#4d114f;cursor:ew-resize;z-index:10;transform:translateX(-50%);', id: 'cs-ba-divider' },
                  components: [{ tagName: 'div', attributes: { style: 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:40px;height:40px;background:#4d114f;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.1em;' }, components: [{ type: 'text', content: '\u2194' }] }]
                },
                { tagName: 'span', attributes: { style: 'position:absolute;top:14px;left:16px;background:rgba(0,0,0,.55);color:#fff;font-size:.78em;font-weight:700;padding:4px 12px;border-radius:20px;z-index:5;text-transform:uppercase;letter-spacing:1px;' }, components: [{ type: 'text', content: 'Before' }] },
                { tagName: 'span', attributes: { style: 'position:absolute;top:14px;right:16px;background:rgba(77,17,79,.8);color:#fff;font-size:.78em;font-weight:700;padding:4px 12px;border-radius:20px;z-index:5;text-transform:uppercase;letter-spacing:1px;' }, components: [{ type: 'text', content: 'After' }] },
              ]
            },
          ]
        },
        { tagName: 'script', components: [{ type: 'text', content: '(function(){var w=document.getElementById("cs-ba-wrap"),a=document.getElementById("cs-ba-after"),d=document.getElementById("cs-ba-divider");if(!w)return;var dr=false;function sp(x){var r=w.getBoundingClientRect(),p=Math.min(Math.max((x-r.left)/r.width*100,0),100);d.style.left=p+"%";a.style.clipPath="inset(0 "+(100-p)+"% 0 0)";}w.addEventListener("mousedown",function(e){dr=true;sp(e.clientX);});document.addEventListener("mousemove",function(e){if(dr)sp(e.clientX);});document.addEventListener("mouseup",function(){dr=false;});w.addEventListener("touchstart",function(e){dr=true;sp(e.touches[0].clientX);},{passive:true});document.addEventListener("touchmove",function(e){if(dr)sp(e.touches[0].clientX);},{passive:true});document.addEventListener("touchend",function(){dr=false;});})();' }] },
      ],
      traits: [
        { type: 'text',  name: 'data-ba-label-before', label: 'Label Links (Before)', changeProp: 0 },
        { type: 'text',  name: 'data-ba-label-after',  label: 'Label Rechts (After)', changeProp: 0 },
  ...
  bm.add('portfolio-section', {
    ...
    content: {
      ...
      tagName: 'section',
      attributes: {
        'data-gjs-type': 'cs-portfolio',
        'data-pf-cols':   '3',
        'data-pf-accent': '#4d114f',
        'data-pf-bg':     '#ffffff',
        'data-pf-radius': '10px',
      ...
      components: [
        { tagName: 'div', attributes: { style: 'max-width:1100px;margin:0 auto;' },
          components: [
            { tagName: 'h2', attributes: { style: 'font-size:2em;font-weight:700;color:#333;margin-bottom:10px;' }, components: [{ type: 'text', content: 'Our Work' }] },
            { tagName: 'p',  attributes: { style: 'color:#999;margin-bottom:16px;' }, components: [{ type: 'text', content: 'A selection of projects we are proud of.' }] },
            { tagName: 'div', attributes: { style: 'display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-bottom:36px;' },
              components: [
                { tagName: 'button', attributes: { onclick: "filterPF(this,'all')",   style: 'padding:7px 18px;border:1px solid #ddd;background:#4d114f;color:#fff;border-radius:20px;cursor:pointer;font-size:.85em;font-weight:600;' }, components: [{ type: 'text', content: 'All' }] },
                { tagName: 'button', attributes: { onclick: "filterPF(this,'web')",   style: 'padding:7px 18px;border:1px solid #ddd;background:#fff;color:#555;border-radius:20px;cursor:pointer;font-size:.85em;' }, components: [{ type: 'text', content: 'Web' }] },
                { tagName: 'button', attributes: { onclick: "filterPF(this,'brand')", style: 'padding:7px 18px;border:1px solid #ddd;background:#fff;color:#555;border-radius:20px;cursor:pointer;font-size:.85em;' }, components: [{ type: 'text', content: 'Branding' }] },
                { tagName: 'button', attributes: { onclick: "filterPF(this,'photo')", style: 'padding:7px 18px;border:1px solid #ddd;background:#fff;color:#555;border-radius:20px;cursor:pointer;font-size:.85em;' }, components: [{ type: 'text', content: 'Photo' }] },
              ]
            },
            { tagName: 'div', attributes: { class: 'cs-pf-grid', style: 'display:grid;grid-template-columns:repeat(3,1fr);gap:20px;' },
              components: [
                { tagName: 'div', attributes: { class: 'cs-pf-item', 'data-cat': 'web',   style: 'position:relative;overflow:hidden;border-radius:10px;background:url(https://picsum.photos/seed/pf1/600/400) center/cover;height:240px;cursor:pointer;' }, components: [{ tagName: 'div', attributes: { class: 'cs-pf-ov', style: 'position:absolute;inset:0;background:rgba(77,17,79,.82);display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;transition:opacity .3s;' }, components: [{ tagName: 'span', attributes: { style: 'color:#fff;font-weight:700;font-size:1.05em;margin-bottom:6px;' }, components: [{ type: 'text', content: 'Project Name' }] }, { tagName: 'span', attributes: { style: 'color:rgba(255,255,255,.7);font-size:.82em;' }, components: [{ type: 'text', content: 'Web Design' }] }] }] },
                { tagName: 'div', attributes: { class: 'cs-pf-item', 'data-cat': 'brand', style: 'position:relative;overflow:hidden;border-radius:10px;background:url(https://picsum.photos/seed/pf2/600/400) center/cover;height:240px;cursor:pointer;' }, components: [{ tagName: 'div', attributes: { class: 'cs-pf-ov', style: 'position:absolute;inset:0;background:rgba(77,17,79,.82);display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;transition:opacity .3s;' }, components: [{ tagName: 'span', attributes: { style: 'color:#fff;font-weight:700;font-size:1.05em;margin-bottom:6px;' }, components: [{ type: 'text', content: 'Brand Identity' }] }, { tagName: 'span', attributes: { style: 'color:rgba(255,255,255,.7);font-size:.82em;' }, components: [{ type: 'text', content: 'Branding' }] }] }] },
                { tagName: 'div', attributes: { class: 'cs-pf-item', 'data-cat': 'photo', style: 'position:relative;overflow:hidden;border-radius:10px;background:url(https://picsum.photos/seed/pf3/600/400) center/cover;height:240px;cursor:pointer;' }, components: [{ tagName: 'div', attributes: { class: 'cs-pf-ov', style: 'position:absolute;inset:0;background:rgba(77,17,79,.82);display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;transition:opacity .3s;' }, components: [{ tagName: 'span', attributes: { style: 'color:#fff;font-weight:700;font-size:1.05em;margin-bottom:6px;' }, components: [{ type: 'text', content: 'Photography' }] }, { tagName: 'span', attributes: { style: 'color:rgba(255,255,255,.7);font-size:.82em;' }, components: [{ type: 'text', content: 'Photo' }] }] }] },
                { tagName: 'div', attributes: { class: 'cs-pf-item', 'data-cat': 'web',   style: 'position:relative;overflow:hidden;border-radius:10px;background:url(https://picsum.photos/seed/pf4/600/400) center/cover;height:240px;cursor:pointer;' }, components: [{ tagName: 'div', attributes: { class: 'cs-pf-ov', style: 'position:absolute;inset:0;background:rgba(77,17,79,.82);display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;transition:opacity .3s;' }, components: [{ tagName: 'span', attributes: { style: 'color:#fff;font-weight:700;font-size:1.05em;margin-bottom:6px;' }, components: [{ type: 'text', content: 'App Design' }] }, { tagName: 'span', attributes: { style: 'color:rgba(255,255,255,.7);font-size:.82em;' }, components: [{ type: 'text', content: 'Web' }] }] }] },
                { tagName: 'div', attributes: { class: 'cs-pf-item', 'data-cat': 'brand', style: 'position:relative;overflow:hidden;border-radius:10px;background:url(https://picsum.photos/seed/pf5/600/400) center/cover;height:240px;cursor:pointer;' }, components: [{ tagName: 'div', attributes: { class: 'cs-pf-ov', style: 'position:absolute;inset:0;background:rgba(77,17,79,.82);display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;transition:opacity .3s;' }, components: [{ tagName: 'span', attributes: { style: 'color:#fff;font-weight:700;font-size:1.05em;margin-bottom:6px;' }, components: [{ type: 'text', content: 'Logo Design' }] }, { tagName: 'span', attributes: { style: 'color:rgba(255,255,255,.7);font-size:.82em;' }, components: [{ type: 'text', content: 'Branding' }] }] }] },
                { tagName: 'div', attributes: { class: 'cs-pf-item', 'data-cat': 'photo', style: 'position:relative;overflow:hidden;border-radius:10px;background:url(https://picsum.photos/seed/pf6/600/400) center/cover;height:240px;cursor:pointer;' }, components: [{ tagName: 'div', attributes: { class: 'cs-pf-ov', style: 'position:absolute;inset:0;background:rgba(77,17,79,.82);display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;transition:opacity .3s;' }, components: [{ tagName: 'span', attributes: { style: 'color:#fff;font-weight:700;font-size:1.05em;margin-bottom:6px;' }, components: [{ type: 'text', content: 'Studio Shots' }] }, { tagName: 'span', attributes: { style: 'color:rgba(255,255,255,.7);font-size:.82em;' }, components: [{ type: 'text', content: 'Photo' }] }] }] },
              ]
            },
          ]
        },
        { tagName: 'style', components: [{ type: 'text', content: '.cs-pf-item:hover .cs-pf-ov{opacity:1!important;}.cs-pf-item.hidden{display:none;}@media(max-width:700px){.cs-pf-grid{grid-template-columns:repeat(2,1fr)!important;}}' }] },
        { tagName: 'script', components: [{ type: 'text', content: 'function filterPF(btn,cat){document.querySelectorAll(".cs-pf-item").forEach(function(i){i.classList.toggle("hidden",cat!=="all"&&i.dataset.cat!==cat);});document.querySelectorAll("button[onclick*=filterPF]").forEach(function(b){b.style.background="#fff";b.style.color="#555";});btn.style.background="#4d114f";btn.style.color="#fff";}' }] },
      ],
      traits: [
        { type: 'select', name: 'data-pf-cols',   label: 'Columns',        changeProp: 0,
          options: [{ id:'2', name:'2 Columns' }, { id:'3', name:'3 Columns' }, { id:'4', name:'4 Columns' }] },
  ...
  bm.add('cookie-banner', {
    ...
    content: {
      ...
      attributes: { 'data-gjs-type': 'cs-cookie',
        'data-cookie-text': 'We use cookies to improve your experience.', 'data-cookie-accept': 'Accept All', 'data-cookie-decline': 'Decline', 'data-cookie-policy': '#' },
      ...
      traits: [
        { type: 'text', name: 'data-cookie-text',    label: 'Banner Text' },
        { type: 'text', name: 'data-cookie-accept',  label: 'Accept Button' },
        { type: 'text', name: 'data-cookie-decline', label: 'Decline Button' },
        { type: 'text', name: 'data-cookie-policy',  label: 'Policy URL' },
      ],
    },
  });
  ...
  bm.add('sticky-header', {
    ...
    content: {
      ...
      attributes: { 'data-gjs-type': 'cs-navbar',
        'data-nav-logo': '&#9632; MyBrand', 'data-nav-bg': '#ffffff', 'data-nav-linkcolor': '#555555' },
  ...
  editor.on('component:add', function(model) {
    ...
    function getSectionModel() {
      var inner = model.components && model.components().find(function(c) { return c.get('tagName') === 'section'; });
      return inner || model;
    }

    model.on('change:attributes', function(m, val, opts) {
    ...
    if ('data-nav-bg' in attrs || 'data-nav-logo' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        if ('data-nav-linkcolor' in changed) {
          ...
          header.components && header.components().each(function(c) {
        ...
        if ('data-nav-logo' in changed) {
          var logoLink = null;
          header.components && header.components().find(function(c) {
            if (c.get('tagName') === 'div') {
              c.components && c.components().each(function(cc) {
    ...
    if ('data-faq-accent' in attrs || 'data-faq-bg' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        if ('data-faq-accent' in changed) {
          ...
          if (el) {
            el.querySelectorAll('span[style*="color"]').forEach(function(s) {
              s.style.color = a['data-faq-accent'];
            });
          }
        }
      });
    ...
    if ('data-nl-btncolor' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        if ('data-nl-btncolor' in changed) {
          function findBtn(cm) {
            cm.components && cm.components().each(function(c) {
    ...
    if ('data-cookie-text' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        if ('data-cookie-text' in changed) {
          ...
          if (p) p.firstChild.textContent = a['data-cookie-text'] + ' ';
        }
    ...
    if ('data-map-url' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        if ('data-map-phone' in changed) {
          ...
          ps.forEach(function(p) { if (p.innerHTML.indexOf('Phone') !== -1) { var t = p.querySelectorAll('br'); if (t[0]) t[0].nextSibling.textContent = a['data-map-phone'] || ''; } });
        }
    ...
    if ('data-ba-accent' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        if ('data-ba-accent' in changed) {
          ...
          var dot     = divider && divider.querySelector('div');
    ...
    if ('data-pf-accent' in attrs) {
      model.on('change:attributes', function(m) {
        ...
        if ('data-pf-accent' in changed) {
          ...
          el.querySelectorAll('.cs-pf-ov').forEach(function(ov) {
            ov.style.background = color.replace('#', 'rgba(') + ',0.82)';
          });
          el.querySelectorAll('button[onclick*=filterPF]').forEach(function(btn) {
            if (btn.style.background !== 'rgb(255, 255, 255)') btn.style.background = color;
          });
        }
        if ('data-pf-radius' in changed) {
          el.querySelectorAll('.cs-pf-item').forEach(function(item) {
            item.style.borderRadius = a['data-pf-radius'] || '10px';
          });
        }
      });
    }
